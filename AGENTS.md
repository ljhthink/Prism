...
- **v1 批次19 两问题修复完成（MCP 命名空间路由幻影修复 / 记忆系统"失忆"三层修复；用户批准调研计划 docs/reports/2026-09-04-memory-and-search-issues-research.md，guardrail 两轮（R2 通过）+ ac-verifier PASS + 全量回归 2596 用例 0 失败 + lint 0 errors）**（2026-09-04）—— 依据批次18 真机测试反馈两问题 + 日志/DB 双取证：
  - **问题1（搜狗/夸克引擎不可用）根因**：Web_Search Server 名规范化含连续下划线（`___Web_Search`），selectMcpServer 用 `substringBefore("__")` 反解命名空间得 ""（起始即命中分隔符）→ 无匹配 → **静默回退第一个启用 Server（Time）** → `mcp____Web_Search__webSearchSogou/Quark/Pro` 全部报"该本地 MCP Server（Time）暂未实现该工具"（data.mdb TOOL 消息实证）；stripNamespace 同源缺陷。**修复**：mcpToolPrefix(serverName) 前缀精确路由（最长前缀优先）+ stripNamespace(serverName) 同源剥离 + **命名空间不匹配返回 null 不再幻影回退**（Skill/跨App 无前缀工具回退逻辑不变）→ 搜狗/夸克/Pro 自动恢复可用
  - **问题2（9 条画像却说"没记忆"）根因（三层）**：①L3 画像**已注入且 LLM 复述出全部条目**（data.mdb AI 回复原文实证）但 section 标题「用户偏好：」被 LLM 当成设备配置而非记忆；②L2 空壳——MemoryFts SQLiteException（疑似崩溃损坏影子表，无自愈；日志只打类名）+ savedCount=0（isImportantTurnPair 过滤）；③身份类提问回写无黑名单。**修复**：L3 section 记忆化（「关于用户的长期记忆」+ 身份类问题必须引用指令 + 低存在感措辞，融合 Letta/Zep/open-webui-memory/Mem0 调研）+ FTS 自愈（rebuildAttempts ≤2 进程级预算 + 成功归零 + deleteDatabase 重建 + 日志带 e.message）+ L2 写入补强（显式"记住 X"直通 + 身份类提问黑名单）
  - **闭环**：guardrail R1（P2-1 自愈风暴缺陷 → 预算模型重设计 / P2-2 自愈零测试 → Robolectric SqliteFtsMemoryIndexSelfHealTest 4 例环境自适应）+ R2 通过（预算模型穷举推演无风暴级漏洞）；ac-verifier PASS；**新增行为规则 BR-interface-022（MCP 路由剥离同源+禁幻影回退）/ BR-data-002（记忆注入自描述+回写过滤）/ BR-concurrency-009（派生索引自愈+预算）**
  - 版本号仍 v1.0.0；真机待补测：搜狗/夸克/Pro 引擎恢复可用、"我是什么样的人"引用长期记忆回答、"记住 X"沉淀 L2、MemoryFts 日志（确认设备端 FTS 失败真因：损坏 vs 模块缺失）
- **v1 批次18 三项真机问题修复完成（熔断外科化 / RAG opt-in / loadSession id 崩溃修复；Bug 修复闭环 §21，guardrail 两轮（R2 通过）+ ac-verifier PASS（追加 14 验收用例）+ 全量回归 2589 用例 0 失败 + lint 0 errors）**（2026-09-03）—— 依据批次17 真机测试反馈三问题 + 日志取证（prism-log-20260903-b17.log）：
  - **问题1+3（任务异常打断）根因**：熔断 MAX_CONSECUTIVE_TOOL_FAILURES=2 + 清空全部工具——19:53 长任务 webSearchPro 连续 2 次失败即终止 TODO 任务；19:37 phone_control__type 连续 2 次**软失败**（⚠️ 输入未生效，isFailureResult 计失败）即终止操控任务。**修复**：分类别阈值（resolveToolFailureThreshold：phone_control__*=4 / 其余=3）+ **外科式单工具禁用**（disabledTools + effectiveTools 过滤，其余工具继续可用）+ 幻觉重调拦截（原生 L576 + 文本路径 L458 双路径一致）+ 全部工具禁尽才纯文本收尾
  - **问题2（未请求知识库被注入）根因**：run-as 实证设备 **prism_rag_config.preferences_pb 缺失**（用户从未切换过）→ decode 缺失回退 AllLibraries（默认开启）→ 每次重启自动全库注入。**修复**：decode 缺失/未知/非法 → **RagTarget.Off**（知识库 opt-in）+ `_ragTarget` 内存初始值同步 Off（guardrail P2-1：消除 init 异步窗口/异常回退到默认开启的缺口）+ RAG 注入成功补结构化日志（target/citations）
  - **附带发现（16:50 FATAL 崩溃）**：`LazyColumn Key "2" already used`——loadSession 恢复 JSON 消息 id 不推进 nextId 计数器 → 新消息 id 与恢复消息冲突。**修复**：loadSession 后 `nextId.set(maxRestoredId + 1)` + isSystemNotice 排除出回退/重试锚点
  - **闭环**：guardrail R1（P2-1 内存默认值 / P2-2 文本路径禁用守卫缺失 → 均修复）+ R2 通过；ac-verifier PASS（新增 SkillExecutorBatch18AcceptanceTest 4 例 + Batch17Test 追加 10 例：阈值边界 / 双路径重调拦截 / loadSession id 唯一性崩溃回归锁 / RAG 组合）；**新增行为规则 BR-error-handling-020（熔断外科式单工具禁用+分类别阈值）/ BR-security-012（知识库注入 opt-in）/ BR-concurrency-008（持久化恢复后推进 id 计数器）**
  - 版本号仍 v1.0.0；真机待补测：长任务爬取失败后换工具继续 / 手机操控 type 软失败不再中断 / 关闭知识库后重启不再注入 / 打开历史会话发消息不崩溃
- **v1 批次17 四项新功能完成（TODO List / 思维链覆写 / 对话回退 / 对话重试；PRD docs/prd-v1-b17-features.md，ADR-043，guardrail 两轮通过（R1 有条件：P2-1 隐私 S1 旁路修复 + P3 清单；R2 复审通过）+ ac-verifier 9/9 PASS（补充 14 边界用例）+ 全量回归 2575 用例 0 失败 + lint 0 errors）**（2026-09-03）—— 用户批准调研报告（docs/reports/2026-09-03-todo-list-research.md）后实施：
  - **A（TODO List，US-1701~1703，ADR-043 D1）**：[TodoLocalToolExecutor]（io/prism/skill/TodoLocalToolExecutor.kt 新）——单工具 `todo_write` 全量替换（Claude Code TodoWrite 同构）：maxItems=8 + 恰好 1 个 in_progress + 违规回灌错误自纠 + 快照回灌（`1.[x] 2.[→] 3.[ ]`）+ TodoListState StateFlow 直推 TodoCard（输入区上方，进度 n/m + 三态图标 + activeForm 高亮，version 原地更新，会话切换 reset）；buildTools 注册（恒启用）+ TODO_GUIDANCE systemPrompt 注入；经 CompositeLocalToolExecutor 组合，SkillExecutor 零改动
  - **B（思维链覆写，US-1704~1705，ADR-043 D2）**：`editAiMessage(messageId, content, thinkingChain)` CAS 原地覆写（不截断后续）+ EditAiMessageSheet 双编辑框（AI 消息「编辑回复」入口）；thinkingChain 经工具回路 reasoning_content 回传生效；业界共识：不回传普通对话请求
  - **C（对话回退，US-1706，ADR-043 D3）**：`rollbackFromUserMessage`（保留锚点 + 截断其后 + 自动重答）+ 确认对话框（另存并回退 / 直接回退）+ `saveCopyAsNewSession`（LibreChat fork 语义副本，防 persistSession 覆盖写回丢原始历史——考古 F 风险闭环）
  - **D（对话重试，US-1707~1708，ADR-043 D4）**：ChatMessage 新增 variants/activeVariantIndex（SillyTavern swipes 同构）+ MessageVariant；`regenerateLastAiMessage`（首次迁移 variants[0] → launchAnswer 继承 → markCompleted 追加新变体，regenTargetId 幂等）+ `switchVariant`（字段拷回保持「消息 ≡ variants[active]」不变量）+ UI ‹2/3› 切换器；variants 不进 LLM 请求（协议零感知）；ChatMessageSerializer encodeDefaults false（空 variants 不落盘防膨胀，旧 JSON 兼容）
  - **guardrail/ac-verifier 闭环**：R1（P2-1 隐私 S1 旁路——stripThinkingChain 未剥 variants[].thinkingChain → 修复 + P3 六项）；R2 复审通过 + R2 后补三处 P3-1 残留一行防护（rollback/regenerate 排除 isSystemNotice 锚点 + UI 操作行隐藏）；ac-verifier 补充 14 边界用例（MAX_VARIANTS 淘汰 / 空内容收敛 / 切换-编辑不变量 / 非法类型防御）
  - 新增行为规则：无（既有 BR 均覆盖）；已知限制：变体 thinkingChain 截断仅 markCompleted 路径（迁移/编辑路径未截断，R2 残留②）；TodoCard 位于输入区上方而非聊天流气泡（PRD 已注明等价语义）
  - 版本号仍 v1.0.0（versionCode 2 未提版）；真机待补测：todo_write 实际触发与卡片更新 / 编辑覆写保存 / 回退+副本 / 重试切换器
- **v1 批次16 MCP 可诊断性 + 手机操控回报完成（US-1601~1605；guardrail 审查通过 + 全量回归 0 失败）**（2026-09-03）—— 针对真机手动测试反馈（MCP 连接不稳定/错误原因不明确/生物识别占位/任务完成无回报）：McpClientManager 陈旧连接自动重试（withRetryOnStaleConnection，listTools/describeTools/diagnose 只读路径；callTool 保持单次尝试防非幂等副作用）+ McpErrorKind 错误分类（明文拦截/超时/拒绝/认证/TLS/协议 → 用户可读中文）+ diagnose 接入设置页测试连接；设置页移除「生物识别解锁」M0 占位行 + 安全拦截提示并入「高危操作拦截」行；PhoneControlCompletionNotifier（LLM 手机操控任务结束且 App 后台时发系统通知，区分完成/异常结束，点击回 App）；sanitizeToolCallPairing 双向配对净化器 + truncateToolResult 20k 截断（真机 RCA：crawl4ai 275KB moegirl Markdown → DeepSeek 400 "insufficient tool messages" 打断，见 docs/reports/2026-09-03-mcp-tool-call-interruption-rca.md）；MCP isError 结果补脱敏日志
...
| v1-B16 | MCP 可诊断性 + 手机操控回报（陈旧连接重试 / 错误分类 diagnose / 生物识别占位移除 / 安全拦截并入高危行 / PhoneControlCompletionNotifier / 配对净化器+20k 截断 RCA 修复） | 真机反馈批次，guardrail 审查通过 + 全量回归 0 失败 + lint 0 errors | 2026-09-03 |
| v1-B17 | 四项新功能（todo_write 任务规划 + TodoCard / 思维链+正文覆写 / 单次回退+另存副本 / swipes 变体重试+切换器） | PRD docs/prd-v1-b17-features.md，ADR-043，guardrail 两轮（R2 通过）+ ac-verifier 9/9 PASS + 全量回归 2575 用例 0 失败 + lint 0 errors | 2026-09-03 |
| v1-B18 | 三项真机问题修复（熔断外科化+分类别阈值 3/4 / RAG opt-in 默认 Off / loadSession nextId 崩溃修复 + isSystemNotice 锚点排除） | Bug 修复闭环 §21，guardrail 两轮（R2 通过）+ ac-verifier PASS（追加 14 用例）+ 全量回归 2589 用例 0 失败 + lint 0 errors | 2026-09-03 |
| v1-B19 | 两问题修复（MCP 命名空间路由前缀精确化+禁幻影回退 / 记忆三层修复：L3 记忆化措辞+FTS 预算自愈+L2 显式直通/身份黑名单） | 用户批准调研计划，guardrail 两轮（R2 通过）+ ac-verifier PASS + 全量回归 2596 用例 0 失败 + lint 0 errors | 2026-09-04 |
